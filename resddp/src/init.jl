...
function init_result(NArea,NHSys,NMaxMStep,NScen,NStage,NK,NLine,NZ)::Result #NZ Added
 ...
 function init_detailed_result(NArea,NMaxMStep,NScen,NStage,NK,NLine,NMaxMod,NZ)::DetailedResult
    ...
    CapZoneUpTable   = zeros(Float64, NZ, NScen, NStage, NK)#ALL Below ADDED
    CapZoneDownTable = zeros(Float64, NZ, NScen, NStage, NK) 
    HydroCapUpTable   = zeros(Float64, NArea, NMaxMod, NScen, NStage, NK)
    HydroCapDownTable = zeros(Float64, NArea, NMaxMod, NScen, NStage, NK)
    WindCapDownTable = zeros(Float64, NArea, NScen, NStage, NK)
    CapDualUpTable = zeros(Float64, NZ, NScen, NStage, NK) 
    CapDualDownTable = zeros(Float64, NZ, NScen, NStage, NK) 
    ObjTable = zeros(Float64, NScen, NStage)  
    ...
    ResAmount = zeros(Float64, NZ, NScen, NStage, NK)
    alpha = zeros(Float64, NScen, NStage)  
    ...
    return DetailedResult(ReservoirTable,HProdTable,MarkTable,FlowTable,DischargeTable,SpillTable,BypassTable,LoadTable,WindTable,PriceTable,
                          RationingTable,DemandUpTable,DemandDnTable,H2StoreTable,H2DisTable,
                          CapZoneUpTable, CapZoneDownTable, HydroCapUpTable, HydroCapDownTable, WindCapDownTable, CapDualUpTable, CapDualDownTable, ObjTable, WaterValueTable,MarkCapUpTablePos,MarkCapDownTablePos,MarkCapUpTableNeg,MarkCapDownTableNeg,SlackUpTable,SlackDownTable,ResAmount,alpha) #Last line ADDED
 end
