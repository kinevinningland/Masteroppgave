   ...
   function Build(t,iWeek,IM,NHSys,HSys,NAreaSys,AreaSys,AMData,MCon,EV,CCR,CCH,CCI,CNS,NCut,NArea,NLine,LineCap,LineLoss,CTI,LEndVal,
      ...
      if LOperatingReserves #ADDED
         ...
         if t==1
            ...
            println("Reserve requirements for stage 1 NI onshore (GWh/step): ", zone_reqs[zi].NI_down * sum(wp_avail[a,k] for a in areas_in_zone[zi] if !(a in zone_reqs[zi].owp_areas_in_zone); init=0.0))
            println("Reserve requirements for stage 1 NI offshore (GWh/step): ", zone_reqs[zi].NI_down_OWP * sum(wp_avail[a,k] for a in zone_reqs[zi].owp_areas_in_zone; init=0.0))
