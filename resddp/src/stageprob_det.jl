...
module StageProbDet
   ...
   function Build(t,iWeek,USMod,AHData,AMData,HSys,MCon,EV,CCR,CCH,CNS,NCut,NHSys,NArea,NLine,LineCap,LineLoss,CTI,LEndVal,LDemandResponse,DR,H2Data,LOperatingReserves,ORData,optimizer) #H2Data, LOperatingReserves & ORData Added
      ...
      @variable(M,0.0 <= res[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=1:NK] <= AHData[iArea].MData[iMod].MaxRes, base_name="res")  #Mm3
      @variable(M,0.0 <= disSeg[iArea=1:NArea,iMod=1:AHData[iArea].NMod,iSeg=1:AHData[iArea].PQData[iMod].NSeg,k=1:NK] <= AHData[iArea].PQData[iMod].DMax[iSeg], base_name="disSeg") #m3s
      @variable(M,0.0 <= dis[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=1:NK] <= InfUB, base_name="dis")                             #m3s                                
      @variable(M,0.0 <= spi[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=1:NK] <= InfUB, base_name="spi")                             #m3s
      @variable(M,0.0 <= byp[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=1:NK] <= InfUB, base_name="byp")                             #m3s
      @variable(M,0.0 <= ghy[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=1:NK] <= InfUB, base_name="ghy")                             #GWh/step
      ...
      @objective(M,MathOptInterface.MIN_SENSE,
                 ...
                 +sum(CNS.CSpi*spi[iArea,iMod,k] for iArea=1:NArea for iMod=1:AHData[iArea].NMod for k=1:NK) 
                 +sum(CNS.CByp*byp[iArea,iMod,k] for iArea=1:NArea for iMod=1:AHData[iArea].NMod for k=1:NK) 
      ...
      @constraint(M,resbalReg0[iArea=1:NArea,iMod=1:AHData[iArea].NMod],res[iArea,iMod,1]
      ...
      @constraint(M,resbalReg[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=2:NK],res[iArea,iMod,k]-res[iArea,iMod,k-1]
      ...
      @constraint(M,discharge[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=1:NK], 
                  dis[iArea,iMod,k]-sum(disSeg[iArea,iMod,iSeg,k] for iSeg=1:AHData[iArea].PQData[iMod].NSeg) == 0.0)

      #HYDROPOWER GENERATION [GWh/Step]
      @constraint(M,prodfunc[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=1:NK],ghy[iArea,iMod,k]
      ...
      @constraint(M,pbal[iArea=1:NArea,k=1:NK],sum(ghy[iArea,iMod,k] for iMod=1:AHData[iArea].NMod) 
      ...
      if LOperatingReserves #ADDED
         ...
         @variable(M, 0 <= cap_hydro_down_mod[iArea=1:NArea,iMod=1:AHData[iArea].NMod,k=1:NK],base_name="cap_hydro_down_mod")
         ...
         @expression(M, cap_up_amount[z = 1:NZ, k=1:NK], 
            ...
            + (sqrt(EEa*zone_reqs[z].MaxLoad/(CNS.MW2GW*CTI.DT)+EEBb^2)-EEBb)*CNS.MW2GW*CTI.DT)                                                   #Load 
         ...
         @constraint(M, reserve_split_up[z=1:NZ, k=1:NK], cap_zone_up[z,k] ==
            sum(sum(cap_hydro_up_mod[iSys, iMod, k] for iMod in 1:AHData[iSys].NMod) for iSys in 1:NArea if (hydrosys_to_area[iSys] in areas_in_zone[z]); init=0.0)
