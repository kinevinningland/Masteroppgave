...
This document describes the extensions added to the ReSDDP model as part of a master's thesis. All additions are marked with `#Added` or `#ADDED` in the source code.

## Overview

The main extension introduces **operating reserve constraints** into the SDDP optimization. When enabled, the model requires that each price zone holds sufficient upward and downward reserve capacity at every time step, supplied by hydropower and wind power. The reserve requirements follow the ENTSO-E methodology.
...
| `NI_up / NI_down` | Nominal imbalance factor for onshore wind |
| `NI_up_OWP / NI_down_OWP` | Nominal imbalance factor for offshore wind |
| `MaxLoad` | Maximum load in zone [MW] |
...
| `LMarkReserves` | Flag for market-based reserves (not yet active) |
...
New module `Reserves` containing `ReadOperatingReserves(...)`. Responsibilities:

- Defines 5 Norwegian price zones (NO1–NO5) and a catch-all "Others" zone
- Assigns all model areas to their respective price zones via `area_to_zone`
- Constructs `ReserveZoneReq` entries with zone-specific RI/NI values
- Builds helper mappings: `hydrosys_to_area`, `h2_to_area`
- Returns a populated `OperatingReserves` object

The module is included in `src/ReSDDP.jl` and called from `load()` in `src/load.jl`.

---

### `src/stageprob_full.jl` and `src/stageprob_det.jl` / `stageprob_det2.jl`
...
A utility module `LeseData` for disabling model components by area index, useful for sensitivity analysis and scenario testing:

| Function | Effect |
|---|---|
| `disable_linecaps_by_area!(model, areas)` | Set transmission capacities to zero for given areas |
| `disable_generation_by_area!(model, areas)` | Zero out hydro production capacities |
| `disable_load_by_area!(model, areas)` | Zero out electricity demand |
| `disable_wind_by_area!(model, areas)` | Zero out wind production |
| `disable_trinn_by_area!(model, areas)` | Disable hydro head-dependent steps |
| `disable_h2caps_by_area!(model, areas)` | Zero out hydrogen storage capacities |

Usage example (see [`examples/4area.jl`](examples/4area.jl)):

```julia
include(joinpath(@__DIR__, "..", "src", "LeseDataset.jl"))
using .LeseData

Base.invokelatest(Main.LeseData.disable_wind_by_area!, model, [21, 22, 23])
```
